...
Utils.lineReader<Monkey>(
  ...
  (match) => {
    ...
    const ifFalseThrowToMonkey =
      match[5] == "false" ? parseInt(match[6]) : null;
    ...
    currentMonkey.divideBy =
      divideBy != null ? divideBy : currentMonkey.divideBy;
...
function normalizeValue(item: number): number {
  const divisibleProduct = monkeys
    .map((m) => m.divideBy)
    .reduce((a, b) => a * b);
